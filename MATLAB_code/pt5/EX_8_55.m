...
Ad=[0.941 0.192; -0.576 0.903]; Bd=[.020; 0.192]; Gd=Bd;
Dx=[1 0;0 1;0 0]; Du=[0;0;1]; Q=[1 0;0 1]; R=0.005; C=[1 0];
dt=0.2; NN=200/dt; Sigw=2.5; Sigv=0.0025*eye(1);
% Find LQOC Gain
P=dare(Ad,Bd,Q,R); L=inv(R+Bd'*P*Bd)*Bd'*P*Ad;
...
randn('state',2^6-1); ww=sqrt(Sigw)*randn(NN,1); vv=sqrt(Sigv)*randn(1,NN);
...
xhat(:,NN+1)=xhat_plus(:,NN+1); uu(:,NN+1)=-L*xhat(:,NN+1);
figure(111); plot(xx(2,:),xx(1,:),'k.');
